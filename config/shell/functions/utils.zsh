...
function upgrade-all() {
  ...
  if command -v brew &>/dev/null; then
    ...
    brew leaves | sort >"$HOME/dotfiles/config/packages/brew_leaves.${host}.txt"
    brew list --installed-on-request | sort >"$HOME/dotfiles/config/packages/brew_installed.${host}.txt"
  fi
  if command -v gh &>/dev/null; then
    gh extension list | awk '{print $3}' | sort >"$HOME/dotfiles/config/packages/gh_extensions.${host}.txt"
  fi
