...
def _fake_runtime_cargo(
    bin_dir: Path,
    *,
    exit_code: int = 0,
) -> None:
    if exit_code:
        _fake_tool(bin_dir, "cargo", f"exit {exit_code}\n")
        return
    built_binary = bin_dir / "built-runtime-binary"
    _fake_tool(
        bin_dir,
        built_binary.name,
        "printf 'generated by built binary\\n'\n",
    )
    _fake_tool(
        bin_dir,
        "cargo",
        "/bin/mkdir -p target/release\n"
        f"/bin/cp {shlex.quote(str(built_binary))} target/release/op-cache\n"
        "/bin/chmod 755 target/release/op-cache\n",
    )
...
def test_runtime_offline_apply_generates_and_removes_owned_shell_files(
    ...
    assert document["summary"] == {"skipped": 18, "succeeded": 11}
...
def test_runtime_build_plan_owns_custom_rust_binaries(tmp_path: Path) -> None:
    ...
    assert "source.atuin" not in steps
    assert "binary.atuin" not in steps
    assert steps["function.atuin"]["status"] == "skipped"
    assert step_names.index("source.op-cache") < step_names.index("binary.op-cache")
    assert not (repo_root / "generated/sources").exists()


def test_runtime_build_installs_op_cache_artifact(
    tmp_path: Path,
) -> None:
    repo_root = tmp_path / "dotfiles"
    home = tmp_path / "home"
    ...
    assert (repo_root / "generated/bin/op-cache").is_file()


def test_runtime_rejected_source_keeps_existing_op_cache_binary(
    tmp_path: Path,
) -> None:
    repo_root = tmp_path / "dotfiles"
    home = tmp_path / "home"
    source_dir = repo_root / "generated/sources/op-cache"
    home.mkdir()
    source_dir.mkdir(parents=True)
    generated_bin = repo_root / "generated/bin"
    generated_bin.mkdir(parents=True)
    _fake_tool(generated_bin, "op-cache")
    original_binary = (generated_bin / "op-cache").read_bytes()

    plan = plan_runtime(
        repo_root,
        home,
        executable_finder=lambda tool: "/fake/cargo" if tool == "cargo" else None,
        build=True,
    )
    ...
    assert results["source.op-cache"].status is RuntimeStatus.FAILED
    assert results["binary.op-cache"].status is RuntimeStatus.SKIPPED
    assert "source.op-cache failed" in (results["binary.op-cache"].reason or "")
    assert (generated_bin / "op-cache").read_bytes() == original_binary
...
def test_runtime_offline_build_is_locked_and_offline(tmp_path: Path) -> None:
    ...
    document = json.loads(completed.stdout)
    steps = {step["name"]: step for step in document["steps"]}
    command = steps["binary.op-cache"]["command"]
...
def test_runtime_online_build_requires_git_to_refresh_existing_source(
    ...
    assert completed.returncode == 0
    steps = {step["name"]: step for step in json.loads(completed.stdout)["steps"]}
    assert steps["source.op-cache"]["status"] == "skipped"
    assert steps["binary.op-cache"]["status"] == "skipped"
    assert steps["binary.op-cache"]["reason"] == "git is not available"
...
def test_runtime_refuses_symlinked_build_sources(tmp_path: Path) -> None:
    repo_root = tmp_path / "dotfiles"
    home = tmp_path / "home"
    external = tmp_path / "external-op-cache"
    home.mkdir()
    (external / ".git").mkdir(parents=True)
    (external / "sentinel").write_text("do not touch\n")
    sources = repo_root / "generated/sources"
    sources.mkdir(parents=True)
    (sources / "op-cache").symlink_to(external)
    ...
    assert report.ok is False
    assert results["source.op-cache"].status is RuntimeStatus.FAILED
    assert "symlink" in (results["source.op-cache"].reason or "")
    assert results["binary.op-cache"].status is RuntimeStatus.SKIPPED
    assert (external / "sentinel").read_text() == "do not touch\n"
    assert (sources / "op-cache").is_symlink()
...
def test_runtime_refuses_symlinked_git_metadata(tmp_path: Path) -> None:
    ...
    source = repo_root / "generated/sources/op-cache"
    ...
    assert results["source.op-cache"].status is RuntimeStatus.FAILED
    assert "Git metadata is a symlink" in (results["source.op-cache"].reason or "")
    assert results["binary.op-cache"].status is RuntimeStatus.SKIPPED
...
def test_runtime_rechecks_build_source_before_execution(tmp_path: Path) -> None:
    ...
    result = next(
        item for item in report.results if item.spec.name == "binary.op-cache"
    )
    assert result.status is RuntimeStatus.FAILED
...
def test_runtime_rechecks_build_source_git_metadata_before_execution(
    ...
    home = tmp_path / "home"
    source_dir = repo_root / "generated/sources/op-cache"
    external_git = tmp_path / "external-op-cache.git"
    home.mkdir()
    (source_dir / ".git").mkdir(parents=True)
    external_git.mkdir()
    (external_git / "sentinel").write_text("do not touch\n")
...
def test_generated_bin_fallback_is_limited_to_self_built_tools(
    ...
    shims.mkdir(parents=True)
    monkeypatch.setenv("MISE_DATA_DIR", str(data_dir))
    _fake_tool(shims, "op-cache")

    finder = repo_aware_finder(
        repo_root,
        {"op-cache": str(shims / "op-cache")}.get,
    )

    # A self-built tool still resolves from generated/bin when its shim is stale...
    assert finder("op-cache") == str(owned_bin / "op-cache")
...
def test_command_environment_prepends_generated_bin_only_for_self_built_tools(
    ...
    op_cache_spec = RuntimeSpec(
        name="binary.op-cache",
        tool="op-cache",
        target=tmp_path / "generated/bin/op-cache",
        command=("op-cache", "--version"),
    )
    ...
    op_cache_path = _command_environment(op_cache_spec, home)["PATH"].split(os.pathsep)
    ...
    assert op_cache_path[0] == owned_bin
    assert owned_bin not in atuin_path
