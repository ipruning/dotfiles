...
def test_runtime_previews_owned_refresh_by_default_without_writing(
    ...
    assert document["summary"] == {"planned": 14, "skipped": 14}
...
def test_runtime_offline_apply_generates_and_removes_owned_shell_files(
    ...
    assert document["summary"] == {"skipped": 18, "succeeded": 10}
    assert (repo_root / "generated/functions/_mise.zsh").read_text() == (
        "generated by mise: activate zsh\n"
    )
    assert (repo_root / "generated/functions/_mise.bash").read_text() == (
        "generated by mise: activate bash\n"
    )
    assert (repo_root / "generated/functions/_mise.nu").read_text() == (
...
def test_runtime_removes_atuin_function_when_host_tool_is_unavailable(
    tmp_path: Path,
) -> None:
    repo_root = tmp_path / "dotfiles"
    home = tmp_path / "home"
...
def test_runtime_failed_update_verification_reports_state_without_rollback(
    tmp_path: Path,
    monkeypatch,
) -> None:
    home = tmp_path / "home"
    ...
    assert f"target revision: {new_revision}" in (report.results[0].reason or "")
    assert "actual revision: wrong-revision" in (report.results[0].reason or "")
    assert f"path: {target}" in (report.results[0].reason or "")
    assert "action: inspect the plugin checkout" in (report.results[0].reason or "")
    assert (target / ".checked-out-revision").read_text().strip() == "wrong-revision"
...
def test_runtime_partial_apply_keeps_exit_code_and_restart_handoff(
    ...
    assert "Refreshed shell runtime is not active in existing shells." in human.stdout
    assert "Restart the affected shell; for Zsh" in human.stdout
...
def test_runtime_refuses_symlinked_git_metadata(tmp_path: Path) -> None:
    ...
    external_plugin_git.mkdir()
    (external_plugin_git / "sentinel").write_text("do not touch\n")
    (plugin / ".git").symlink_to(external_plugin_git)
...
def test_shim_aware_finder_rejects_stale_mise_shims(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    finder = shim_aware_finder(executables.get)
