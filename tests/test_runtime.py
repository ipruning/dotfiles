...
def _fake_runtime_cargo(
    bin_dir: Path,
    atuin_output: str = "generated by built atuin",
    *,
    exit_code: int = 0,
) -> None:
    if exit_code:
    ...
    _fake_tool(
        ...
        f'/bin/cp {shlex.quote(str(built_binary))} "target/release/$binary"\n'
...
def test_runtime_build_plan_owns_custom_rust_binaries(tmp_path: Path) -> None:
    ...
    assert step_names.index("source.atuin") < step_names.index("binary.atuin")
    assert step_names.index("binary.atuin") < step_names.index("function.atuin")
    assert not (repo_root / "generated/sources").exists()


def test_runtime_build_installs_artifacts_and_generates_atuin_function(
    tmp_path: Path,
) -> None:
    repo_root = tmp_path / "dotfiles"
    home = tmp_path / "home"
    ...
    assert steps["function.atuin"]["status"] == "succeeded"
    assert (repo_root / "generated/functions/_atuin.zsh").read_text() == (
        "generated by built atuin\n"
    )


def test_runtime_build_regenerates_atuin_function_after_replacing_binary(
    tmp_path: Path,
) -> None:
    repo_root = tmp_path / "dotfiles"
    home = tmp_path / "home"
    ...
    _fake_tool(generated_bin, "atuin", "printf 'generated by old atuin\\n'\n")
    generated_function = repo_root / "generated/functions/_atuin.zsh"
    generated_function.parent.mkdir(parents=True)
    generated_function.write_text("generated by old atuin\n")
    _fake_runtime_cargo(bin_dir, "generated by new atuin")
    ...
    assert steps["binary.atuin"]["status"] == "succeeded"
    assert steps["function.atuin"]["status"] == "succeeded"
    assert generated_function.read_text() == "generated by new atuin\n"


def test_runtime_failed_build_keeps_existing_atuin_runtime_coherent(
    tmp_path: Path,
) -> None:
    repo_root = tmp_path / "dotfiles"
    home = tmp_path / "home"
    ...
    generated_bin.mkdir(parents=True)
    _fake_tool(generated_bin, "atuin", "printf 'generated by old atuin\\n'\n")
    original_binary = (generated_bin / "atuin").read_bytes()
    generated_function = repo_root / "generated/functions/_atuin.zsh"
    generated_function.parent.mkdir(parents=True)
    generated_function.write_text("generated by old atuin\n")
    _fake_runtime_cargo(bin_dir, exit_code=7)
    ...
    assert completed.returncode == 1
    steps = {step["name"]: step for step in json.loads(completed.stdout)["steps"]}
    assert steps["binary.atuin"]["status"] == "failed"
    assert steps["function.atuin"]["status"] == "skipped"
    assert steps["function.atuin"]["reason"].startswith("binary.atuin failed")
    assert (generated_bin / "atuin").read_bytes() == original_binary
    assert generated_function.read_text() == "generated by old atuin\n"


def test_runtime_rejected_source_keeps_existing_atuin_runtime_coherent(
    tmp_path: Path,
) -> None:
    repo_root = tmp_path / "dotfiles"
    home = tmp_path / "home"
    ...
    assert results["function.atuin"].status is RuntimeStatus.SKIPPED
    assert "binary.atuin was not refreshed" in (results["function.atuin"].reason or "")
    assert (generated_bin / "atuin").read_bytes() == original_binary
    assert generated_function.read_text() == "generated by old atuin\n"
