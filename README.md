...
- **`home/` is a Mackup-managed snapshot tree**. It is the _materialized output_ of backup/restore (i.e., copied in/out by Mackup). Treat it as “backed-up artifacts”, not the canonical authoring location. Exception: tracked shell bootstrap files under `home/` (`.zshenv`, `.zprofile`, `.zshrc`) are intentionally edited when changing Mackup-restored shell startup behavior; ignored `home/.zshenv.private` is generated locally from `home/.zshenv.private.tpl` when 1Password is available.
...
2. **`home/` is not canonical source**
   ...
   - Exception: tracked shell bootstrap files under `home/` (`.zshenv`, `.zprofile`, `.zshrc`) are intentionally edited when changing Mackup-restored shell startup behavior. Ignored `home/.zshenv.private` is generated locally from `home/.zshenv.private.tpl` when 1Password is available.
...
4. **Secrets never enter Git**
   ...
   - Ignored materialized files such as `home/.zshenv.private` may be generated locally from templates.
...
- ignored local secret materializations such as `.zshenv.private`, generated from tracked templates before Mackup restore
...
- `restore` optionally generates ignored private materializations such as `home/.zshenv.private`, then pulls the Mackup snapshot into the correct locations
